...
const deleteChripById = `-- name: DeleteChripById :one
DELETE FROM chirps
WHERE id = $1
RETURNING id, created_at, updated_at, body, user_id
`

func (q *Queries) DeleteChripById(ctx context.Context, id uuid.UUID) (Chirp, error) {
	row := q.db.QueryRowContext(ctx, deleteChripById, id)
	var i Chirp
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
